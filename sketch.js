...
let playerAnis = {
  idle: { row: 1, frames: 4, frameDelay: 10 },
  run: { row: 1, frames: 4, frameDelay: 3 },
  jump: { row: 1, frames: 3, frameDelay: Infinity, frame: 0 },
  attack: { row: 1, frames: 6, frameDelay: 2 },
  hurtPose: { row: 1, frames: 4, frameDelay: Infinity },
...
const SNAKE_OFFSET_Y = -9; // snake sprite lives in bottom 14px of 32px frame, pull up to align
...
let sfxJump, sfxHurt, sfxCollect;
let lastScore = null;
let lastHealth = null;
...
function preload() {
  playerImg = loadImage("assets/characters.png");
  boarImg = loadImage("assets/characters.png");
  ...
  sfxJump = loadSound("assets/jump.wav");
  sfxHurt = loadSound("assets/hurt.wav");
  sfxCollect = loadSound("assets/collect.wav");
}
...
function draw() {
  ...
  for (const layer of bgLayers) {
    const img = layer.img;
    const w = img.width;
    // camera.x is the world center the camera looks at; starts at VIEWW/2
    const camOffset = (camera.x || VIEWW / 2) - VIEWW / 2;
    let x = Math.round(-camOffset * layer.speed) % w;
    if (x > 0) x -= w;
    // tile horizontally in case image is narrower than viewport
    for (let tx = x; tx < VIEWW; tx += w) image(img, tx, 0);
  }
  ...
  // snap boars too so they don't jitter
  const boarSnaps = [];
  for (const e of boar) {
    boarSnaps.push({ e, x: e.x, y: e.y });
    e.x = Math.round(e.x);
    e.y = Math.round(e.y);
  }
  ...
  // restore boar physics positions
  for (const { e, x, y } of boarSnaps) {
    e.x = x;
    e.y = y;
  }
...
function restartGame() {
  ...
  boar.anis.w = FRAME_W;
  boar.anis.h = FRAME_H;
  boar.anis.offset.y = SNAKE_OFFSET_Y;
  ...
  for (const s of boarSpawns) {
    ...
    e.anis.w = FRAME_W;
    e.anis.h = FRAME_H;
    e.anis.offset.y = SNAKE_OFFSET_Y;
    e.addAnis(boarAnis);
...
function makeWorld() {
  ...
  // --- BACKGROUND PARALLAX ---
  bgLayers = [
    { img: bgFarImg, speed: 0.15 },
    { img: bgMidImg, speed: 0.35 },
    { img: bgForeImg, speed: 0.55 },
  ];
}
